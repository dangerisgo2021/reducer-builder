		...
		value: function build() {
			...
			if ((0, _lodash.isEmpty)(self.nameToChild)) {
				...
				return function () {
					...
					var reducer = self.typeToReducer[action.type];
				...
				return function () {
					...
					var rootReducer = self.typeToReducer[action.type];

					var nextRootState = rootReducer ? rootReducer(state, action) : state;

					var hasChanged = state === nextRootState;

					var nextCombinedReducersState = Object.keys(self.keyToReducer).reduce(function (nextState, key) {

						var reducer = self.keyToReducer[key];
						var stateForKey = state[key];
						var nextStateForKey = reducer ? reducer(previousStateForKey, action) : stateForKey;
